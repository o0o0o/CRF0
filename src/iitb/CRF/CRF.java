...
public class CRF implements Serializable {
    ...
    // FeatureGenerator is an interface.
    FeatureGenerator featureGenerator;
    // EdgeGenerator is a class
    EdgeGenerator edgeGen;
    // HistoryManager is a class
    HistoryManager histMgr;
    // CrfParams is a class
    public CrfParams params;
    // Viterbi is a Class, seems very important!
     ...
     */
    // the construction method. but only formally
    ...
    // the real construction method.
    public CRF(int numLabels, int histsize, FeatureGenerator fgen, java.util.Properties configOptions) {
        // need to get the defnition of HistoryManager.
        histMgr = new HistoryManager(histsize,numLabels);
        // featureGenerator can be got from HistoryManager.getFeatureGen(fgen)
        // fgen is a FeatureGenerator, get a new FeatureGenerator by passing an old featureGenerator
        featureGenerator = histMgr.getFeatureGen(fgen);
        // local field numY is assigned by the HistoryManager's field numY.
        numY = histMgr.numY;
        // pass a Properties Class to CrfParams
        params = new CrfParams(configOptions);
        // one can get an Edge generator from the HistoryManager.
        edgeGen = histMgr.getEdgeGenerator();
        // getViterbi function is from the local definition.
    ...
    public void read(String fileName) throws IOException {
        ...
        int numF = Integer.parseInt(in.readLine());// get the total counts of double numbers.
        lambda = new double[numF];
        int pos = 0;// index for pos array.
        String line;// tempora storage for String, as long as not null
        while((line=in.readLine())!=null) {
            lambda[pos++] = Double.parseDouble(line);
        }
    }
    protected Trainer dynamicallyLoadedTrainer() {
        // trainerType is a String. judge according to the String start.
        if (params.trainerType.startsWith("load=")) {
            try {
                Class c =  Class.forName(params.trainerType.substring(5));
                // an instance of some Class can get a constructor to get new instances.
                Constructor constr = c.getConstructor( new Class[] { CrfParams.class } );
                // use the constructor to get new instances. given by some params.
                // need to figure out what the "new Object[] {params}" mean
    ...
    protected Trainer getTrainer() {
        // it is possible to get a null trainer.
    ...
    public Viterbi getViterbi(int beamsize) {
        // return either SegmentViterbi or a basic Viterbi
